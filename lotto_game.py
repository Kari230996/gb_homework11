...
'''Создадим функцию для случайного боченка из мешки, а также для вычета его количество'''
def generate_unique_numbers(count, minbound, maxbound):
    if count > maxbound - minbound + 1:
        raise ValueError('Неправильный ввод параметров')
    ret = []
    while len(ret) < count:
...
class Card:
    ...
    def __init__(self):
        ...
        for i in range(0, self.__rows):
            tmp = uniques[self.__nums_in_row * i: self.__nums_in_row * (i + 1)]
    ...
    def cross_num(self, num):
        for index, item in enumerate(self.__data):
            if item == num:
                ...
                return
        raise ValueError(f'Нет такого номера на карте: {num}')
...
'''Через скрипт запустим нашу игру'''
